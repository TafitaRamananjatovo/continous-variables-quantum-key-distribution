...
def generate_ldpc_code(n, q, r_code):
    """
    ...
    wr = int(np.round(wc / (1 - r_code)))  # Row weight of the low-density parity-check matrix
    m = int(np.round(n * wc / wr))  # Number of rows of the low-density parity-check matrix
    k = int(np.round(n * r_code))  # Information bits of the low-density parity-check matrix
    r_design = 1 - wc / wr  # The true rate of the code, which should be very close to the code rate from the data
    np.testing.assert_almost_equal(r_design, r_code, decimal=2, err_msg="The error between the LDPC code rate and the "
    ...
    """if load:
        ...
        if int(vals[0].replace("codes/", "")) != n:
            ...
            m = code.shape[0]
            vals = get_values(code_sparse)
    else:"""
    code, vals = create_random_regular_code(n, m, wc, wr, q)
    code_sparse = csr_matrix(code)
...
def create_random_regular_code(n, m, c, r, q):
    ...
    if n < 10 ** 4:
        check_matrix_rank(h, n)  # Find if the matrix has full rank
...
def check_row_weights(h, m, r):
    ...
    """
    row_weight = np.empty(m, dtype=np.uint32)
    row_error = 0
    for i in prange(m):
        row_weight[i] = np.count_nonzero(h[i])
...
def check_column_weights(h, n, c):
    ...
    """
    col_weight = np.empty(n, dtype=np.uint32)
    col_error = 0
    for i in prange(n):
        col_weight[i] = np.count_nonzero(h.T[i])
...
def get_values(h: csr_matrix):
    ...
    for i in range(len(r)):
        vals[(r[i], c[i])] = h[r[i], c[i]]

    return vals


def get_nodes(n, m, h: csr_matrix):
    """
    ...
    c_lil = h.tolil().rows
    c_lil_t = h.transpose().tolil().rows

    # Get the indices of CN-to-VN messages
    for i in range(m):  # For every row of the VN-to-CN messages array
        rows.append(List(c_lil[i]))  # Get the VNs connected to a certain CN
